...
public class Painter extends Thread{
	...
	public Painter(String path, int id){
		inputPath = path;
		this.id = id;
	}
	public void run(){
		...
		int grayPixel;
		int textChar;
		ArrayList<Integer> gspixels = new ArrayList<Integer>();
			...
			for(int width=0;width<imgx;width+=8){
				for(y=0;y<18;y++){
					for(x=0;x<8;x++){
						...
						grayPixel = (int) ((red*0.299)+(green*0.587)+(blue*0.114));
						gspixels.add(grayPixel);
					}
				}
				textChar = calcAvg(gspixels);
				gspixels = new ArrayList<Integer>();
		...
		outputGraphics.setPaint (new Color (0,0,0));
		outputGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,RenderingHints.VALUE_TEXT_ANTIALIAS_LCD_HRGB);
		outputGraphics.setRenderingHint(RenderingHints.KEY_FRACTIONALMETRICS,RenderingHints.VALUE_FRACTIONALMETRICS_ON);
	...
	private static int calcAvg(ArrayList<Integer> pixels) {
		  double sum = 0;
		  if(!pixels.isEmpty()) {
